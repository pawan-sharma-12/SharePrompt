...
const Nav = () => {
  ...
  return (
    <nav className="flex-between w-full mb-16 pt-3">
      ...
      <Image src="/assets/images/logo.svg" alt='Promptopial Logo'
       ...
       <div className="sm:flex hidden">
        {session?.user ? (
          <div className='flex gap-3 md:gap-5'>
            ...
            className='round-full' alt='profile'></Image>
            </Link>
       ...
       <div className="sm:hidden flex relative">
        {
          session?.user ? (
            ...
            className='round-full' alt='profile' 
          ...
          ) : (<>
                {
                  providers && Object.values(providers).map((provider) => (
                    <button type = "button" key = {provider.name} onClick={()=>signIn(
                      provider.id
                    )} className="black_btn">SingIn</button>
                  ))
                }
          </>)
